...
def _sync_locations_for_filament_system_type(session: Session, printer: Printer, new_type: str) -> None:
    ...
    back and forth between ams/external_spool idempotent and safe (no risk
    of orphaning an active SpoolAssignment or Sensor by removing the
    Location it points to). storage_only/manual leave Locations untouched.
    """
    if new_type == "ams":
        existing_slots = {
            row.slot_index
