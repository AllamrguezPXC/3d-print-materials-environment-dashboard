...
| 6 | GitHub MCP Integration | Curso 2 | 🟡 | No GitHub MCP server was reachable in the Claude Code sessions that did this work (confirmed again during this review via `ToolSearch`/config inspection — a `github` MCP server *is* configured in the user's Claude Code settings, but under a project-path entry with different drive-letter casing than the active session, so it never loaded). Real, documented substitute: `gh issue create`/`gh issue close` (Issue #1) and `gh pr create`/`gh pr merge` (PR #2) — both real actions against the live GitHub repo | Marked partial (not ❌) because the assignment explicitly accepts a documented, clearly-explained substitute when the tool is unavailable — which is exactly this case. Reconnecting the MCP server (session restart with matching path casing) would let a future session upgrade this to a literal MCP-driven action |
...
| Uso de Claude Code | 40% | ✅ (7/8 full, 1/8 documented-substitute) | See §3 — every theme has concrete, inspectable evidence in the repo |
...
- **GitHub MCP**: configured in the user's Claude Code settings but not active in the sessions that
  built this project, due to a project-path casing mismatch (see §3, item 6). The `gh` CLI
  substitute is real and fully documented, per the assignment's own allowance for this case.
